...
top6 = sorted(seoul_districts, key=lambda x: x['count'], reverse=True)[:6]

nameArr = []
countArr = []
for district in top6:
    nameArr.append(district["name"])
    countArr.append(district["count"])

def func(pct):
    total = sum(countArr)
    val = int(pct * total / 100)  # 퍼센트 값을 실제 값으로 변환 후 정수로 변환
    return f'{val}'  # 정수 값만 반환

plt.pie(countArr,autopct = func,  labels = nameArr)
plt.show()
